...
def load_model():
    ...
    input = Input(shape=(32, 600, 1), name='the_input')
    modelPath = r'/home/huangzheng/ocr/OCR/model/ocr2_model.hdf5'
...
def predict(img_path, basemodel, thresholding=160):
    ...
    t_img = np.zeros((32, 600))
    t_img[:, :int(rate * 32)] = img
    X = np.array([t_img])
    X = X.reshape((1, 32, 600, 1))

    y_pred = basemodel.predict(X)
    out = np.argmax(y_pred, axis=2)[0]
    y_pred = y_pred[:, :, :]
    start = time.time()
    ...
    out = decode(out)
    out = u''.join([id_to_char[x] for x in out])
    out = out.replace("卍", "")
    return out, t_img
