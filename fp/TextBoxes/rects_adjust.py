...
###
###
def rects_adjust(im, rcts, fore_thres=0.3, extend_max_ratio=0.3, tlh_ratio_thres=0.5):
    ...
    for i in range(len(_rcts)):
        ...
        _rl = r.copy()
        left = _line_scan_1eft(bn_img, left, int(r[3] * extend_max_ratio), fore_thres)
        _rl[2] += r[0] - left[0][0]
        _rl[0] = left[0][0]

        _rr = r.copy()
        right = _line_scan_right(bn_img, right, int(r[3] * extend_max_ratio), fore_thres)
        #_rr[2] = right[0][0]-r[0]
        ...
        _rn = bn_img[cmrcts[1]:cmrcts[1] + cmrcts[3], cmrcts[0]:cmrcts[0] + cmrcts[2]]
