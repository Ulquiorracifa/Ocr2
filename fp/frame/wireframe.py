...
def _image_corner_point(point, image_size, ratio=0.02):
    '''
    args
...
class Detect(object):
    ...
    def __call__(self, image):
        ...
        major_points = np.array([p for p in major_points
                                 if not _image_corner_point(p, image_size)])
        # print(major_points)
        ...
        if self.debug is not None:
            ...
            disp = visualize.points(disp, major_points, radius=9, color=(0, 0, 255))
